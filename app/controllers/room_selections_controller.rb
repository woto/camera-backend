class RoomSelectionsController < ApplicationController
  ...
  def load_events_for_current_page
    @events = events_scope.order(captured_at: :desc).includes(captures: [ { small_thumbnails_attachments: :blob }, { large_thumbnails_attachments: :blob } ]).page(params[:page]).per(5)
  end
end
