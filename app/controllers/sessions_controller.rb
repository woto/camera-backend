class SessionsController < ApplicationController
  ...
  def create
    if (user = User.find_by(username: params[:username])) && user.authenticate(params[:password])
      ...
      redirect_back fallback_location: events_path, notice: t("auth.login_success"), status: :see_other
    else
  ...
  def destroy
    reset_session
    redirect_back fallback_location: events_path, notice: t("auth.logout_success"), status: :see_other
  end
end
